...
export const AiAssistantCard: React.FC<AiAssistantCardProps> = ({ assistant }) => {
  ...
  return (
    <motion.div 
      className={`premiumCard premiumCardHero ${styles.aiCardHero}`}
    ...
    >
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '20px' }}>
        <motion.div
          style={{
            position: 'absolute',
            width: '144px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', zIndex: 2 }}>
        <h3 className="text-subheading" style={{ fontSize: '1.35rem' }}>AI Career Match Assistant</h3>
        <p className="text-description" style={{ fontSize: '0.825rem', maxWidth: '280px', margin: '0 auto', textAlign: 'center' }}>
      ...
      >
        <span style={{ fontWeight: 700, color: 'var(--color-primary)', display: 'block', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Visionix AI</span>
        "{activeMessage}"
      </div>
      ...
      <motion.button 
        ...
        whileTap={{ scale: 0.98 }}
        style={{ width: '100%', zIndex: 2, padding: '14px 28px', fontSize: '0.9rem' }}
      >
